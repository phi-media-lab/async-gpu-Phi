// should_compile: a real Low task may await another spawned Low typed task.

use gpu_runtime::executor::{ExecutorError, GpuExecutor};
use gpu_runtime::priority::{level::Low, CompletionCell, PriorityClass};
...
pub unsafe fn build_graph(executor: &GpuExecutor) -> Result<(), ExecutorError> {
    let target =
        executor.spawn_typed(PriorityClass::<Low>::new(), &TARGET_RESULT, |_| async { 7 })?;
